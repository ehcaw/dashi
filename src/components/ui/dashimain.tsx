...
export default function AnimatedVectorBox({ 
  ...
  useEffect(() => {
    console.log('Component state - isActivated:', isActivated, 'isExpanded:', isExpanded, 'isActivatedState:', isActivatedState);
    
    const handleKeyDown = (event: KeyboardEvent) => {
      console.log('Key pressed:', event.key, 'KeyCode:', event.keyCode, 'Meta:', event.metaKey, 'Alt:', event.altKey, 'Ctrl:', event.ctrlKey);
  ...
  }, [isActivated, isExpanded, isActivatedState]);
  ...
  const handleChatSubmit = (e: React.FormEvent) => {
    ...
    if (chatInput.trim()) {
      ...
      setTimeout(() => {
        const dashiResponse = { text: "idk", isUser: false };
        setChatMessages(prev => [...prev, dashiResponse]);
      }, 500);
    }
  };

  // Determine current width, height, and position based on states
  const currentWidth = isResetting ? (isActivatedState ? 400 : (isExpanded ? 300 : 100)) : (isActivatedState ? 400 : (isExpanded ? 300 : (isActivated ? 100 : 100)));
  const currentHeight = isResetting ? (isActivatedState ? 600 : height) : (isActivatedState ? 600 : height);
  const currentLeft = isResetting ? (isActivatedState ? 'calc(50% - 200px)' : '0px') : (isActivatedState ? 'calc(50% - 200px)' : '0px');
    ...
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      <motion.div
        animate={{ 
          ...
          left: currentLeft,
          opacity: isResetting ? 0 : (isInitialAnimation ? 0.3 : (isActivated ? 0.3 : 0.1))
        }}
        transition={{ 
      ...
      {isActivatedState && (
        <motion.div
          ...
          style={{
            ...
            left: currentLeft === 'calc(50% - 200px)' ? 'calc(50% - 200px)' : '0px',
            width: '400px',
            height: '600px',
            display: 'flex',
            flexDirection: 'column',
      ...
      <motion.div
        ...
        animate={{ 
          left: isResetting ? (isActivatedState ? 'calc(50% - 180px)' : (hasMovedLeft ? `calc(${leftMargin}px + 20px)` : '65%')) : (isActivatedState ? 'calc(50% - 180px)' : (hasMovedLeft ? `calc(${leftMargin}px + 20px)` : '65%')),
          top: isResetting ? (isActivatedState ? '30px' : '65%') : (isActivatedState ? '30px' : '65%'),
          transform: isResetting ? (isActivatedState ? 'none' : (hasMovedLeft ? 'translateY(-50%)' : 'translate(-50%, -50%)')) : (isActivatedState ? 'none' : (hasMovedLeft ? 'translateY(-50%)' : 'translate(-50%, -50%)')),
          opacity: isResetting ? 0 : (isInitialAnimation ? 1 : (isActivated ? 1 : 0.1))
        }}
        transition={{ 
